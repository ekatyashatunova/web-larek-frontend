...
#### Класс Page
Отвечает за отображение контента на главной странице: каталог карточек товаров + шапка сайта с логотипом и кнопкой корзины.

Поля класса:
- headerContainer: <HTMLElement> - шапка страницы  с логотипом
- gallery: <HTMLElement> - каталог карточек товаров на странице
- buttonBasket: <HTMLButtonBasket> - кнопка для корзины
- basketCounter: <HTMLElement> - отображение счетчика товаров в корзине
- events: IEvents - брокер событий

Методы:
- open(): void - открытие модального окна корзины
- render(items: IProductItem[]): <HTMLElement> - возвращает массив заполненных карточек с товарами
...
- deleteProductcard(id: string): void - удаление товара из корзины
- clearBasket(): void - очистить корзину
...
Методы:
- render(): <HTMLFormElement> - возвращает элементы формы
