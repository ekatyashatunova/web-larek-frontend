...
- product: IProductItem[] - хранение массива карточек товаров
...
В классе есть следующие методы:
- setProducts(card: IProductItem): void - сохранить/установить массив карточек с сервера
- getProducts(): IProductList - возвращает массив карточек
...
- setGallery(items: IProductItem[]): void - устанавливает каталог карточек товаров
...
Устанавливает обработчик события на кнопку 'Далее'.
Также есть метод get form, который позволяет получить элемент формы.
